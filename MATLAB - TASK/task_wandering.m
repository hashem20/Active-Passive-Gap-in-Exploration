classdef task_wandering < handle
    ...
    methods
        ...
        function obj = task_wandering(stimdir, sounddir, datadir, savename)
        ...
        function mu = makeChangePointRewards(obj, h, T, mu_min, mu_max)
            ...
            for s = unique(seg)'
                ind = seg == s;
                mu(ind) = rand(1)*(mu_max-mu_min) + mu_min;
            end
        ...
        function instructionList(obj)
            ...
            i=i+1; ev{i} = 'default';       iStr{i} = 'Most of the time the average payoff from each slot machine is fixed, but on any given play there is variability in the exact amount.';
            ...
            i=i+1; ev{i} = '90';            iStr{i} = 'To make things even more difficult, at random times the average reward of either slot machine will change to be anywhere between 0 and 100.  For example, here the mean reward has shifted up and the slot machine is now paying out 90 points';
            i=i+1; ev{i} = 'bandits_ex';    iStr{i} = 'We won''t tell you when the mean reward changes and either option could change at any given time, so while you''re playing the left slot machine the right slot machine could have changed.';
        ...
        function makeBandits(obj, nBandits)
            ...
            for i = 1:nBandits
                ban(i) = bandit_playable(obj.window, obj.stimdir, ['bandit' num2str(i) '.png'], i, 'changed.png');
        ...
        function makeGameParameters(obj)
            ...
            % drift condition
            var(i).x = [0.2 0.1 0.05 0.02];
            var(i).type = 1;
            i=i+1;
            
            [var, T, N] = counterBalancer(var, 10);
            ...
            for j = 1:T
                ...
                % make means
                for k = 1:nBandits
                    mu(:,k) = obj.makeChangePointRewards(h_vals(j), gameLength, mu_min, mu_max);
        ...
        function run(obj, numBlocks, numGames)
            ...
            for blockNum = 1:numBlocks
                ...
                %%%% NOTE BIT LINE ONLY WORKS FOR numGames == 1 !!!!!!!!!!!
                X = vertcat(obj.game(gameCount).rewards);
                MAX_R = sum(max(X'));
